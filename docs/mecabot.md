...
4. Les garages sont classés avec un score mêlant spécialité, compatibilité de marque, note, statut vérifié et proximité lorsque la zone saisie correspond à une ville ou un département présent dans le catalogue.
...
La carte interactive repose sur MapLibre GL JS et le fond libre OpenFreeMap. Elle ne nécessite ni compte cartographique ni token. Si la bibliothèque ou les tuiles ne sont pas disponibles, l'application conserve un aperçu schématique local des garages comme solution de repli.
...
Cette couche reste indépendante du modèle génératif pour les décisions sensibles : le type de réponse, l'urgence, la confiance, les spécialités, les bornes de coût et le filtrage des garages sont contrôlés par le moteur métier. Quand `GROQ_API_KEY` est configurée, Groq enrichit uniquement le résumé, les hypothèses et les précautions d'un diagnostic déjà validé par les règles locales. Groq ne peut donc ni modifier l'urgence, ni choisir un garage, ni produire une fourchette de prix. En cas d'absence de clé, d'erreur, de réponse invalide ou de délai dépassé, MecaBot revient automatiquement au moteur local.
...
Les résultats de tests à jour sont consignés dans le plan de tests après chaque exécution complète. Une suite adversariale vérifie notamment les injections de prompt, les demandes dangereuses, l'impossibilité de modifier l'urgence et l'interdiction d'inventer des prix ou des garages.
